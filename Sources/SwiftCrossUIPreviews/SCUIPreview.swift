    ...
    ///
    /// SwiftCrossUI views don't conform to `SwiftUI.View`, so Xcode's `#Preview`
    /// macro can't render them directly. Wrapping a view in ``SCUIPreview``
    /// bridges the two worlds by rendering the view with ``AppKitBackend`` and
    /// exposing the resulting `NSView` to SwiftUI.
    ///
    /// ```swift
    /// #Preview {
    ///     SCUIPreview {
